...
> __NOTE:__  Simplest behaviour can be observed by calling a `map` function on a vector. Vector will be authomatically transformed to list and operation will be performed on each element of this list (i.e. on each element of the original vector). 
>
> If you specify the dataframe in place of `<DATA>`, because dataframe is a list of columns, function will be performed on columns.
>
...
As you have seen above, we can use `map()` to apply a linear model to element of our complex `data` column. We will capute the results in a newly created variable in the `by_country` data frame with `mutate()`.
