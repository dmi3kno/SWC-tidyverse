...
ggplot(data = gapminder) + 
  geom_point(mapping = aes(x = gdpPercap/10^3, y = lifeExp, size=pop/10^6, color=continent)) +
  geom_text(mapping = aes(x = 40, y = 30), label=as.character(gapminder$year), size = 16, color="grey") +
  scale_x_log10() +
  labs(title="Life Expectancy vs GDP per capita over time",
       subtitle="In the past 50 years, life expectancy has improved in most countries of the world",
       caption="Source: Gapminder foundation, https://www.gapminder.org/data/",
...
# install.packages("sf")
library(sf)
world_map <- sf::st_read("data/TM_WORLD_BORDERS-0.3/", quiet = TRUE)
ggplot(world_map) +
  geom_sf(aes(fill = AREA))

```

We can merge this data with out own, by ensuring the map data has a _common variables_ as our statistics data file from gapminder. In this case, our world map has a column called `NAME` that corresponds with a column in the gapminder data called `country`, so we just create a duplicate column in world_map called country, and merge the two data sets together. We can then plot different aesthetics on the map, based on the data from gampinder.

```{r}
world_map$country = world_map$NAME
gapminder_map = left_join(gapminder, world_map)
