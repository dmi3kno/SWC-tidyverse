...
First of all, lets look at the chart and assess what data we might need to plot it. The chart says dataset includes selected (few) African countries with at least 2 mln baby deaths in 2007. Let's see if we can identify those cases
...
Note that `pop_mln` and `gdp_bln` are two new variables. Also `continent` and `pop` are missing. Lets mutate our data to accommodate those:
...
Now, we would like one of the layers (`geom_text`) to use this small dataset of only 6 records, while the rest of the layers should use the complete dataset. Turns out you can accommodate it by specifying own `data` parameter in a layer and utilizing a `dplyr` dot placeholder as a starting point for refining the dataset for the purpose of one layer.
